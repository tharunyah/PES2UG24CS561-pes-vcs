...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // Step 1: prepare header "<type> <size>\0"
    const char *type_str =
        (type == OBJ_BLOB) ? "blob" :
    ...
    // Step 4: if object already exists, stop
...
int object_read(const ObjectID *id, ObjectType *type_out,
    ...
    size_t type_len = space - (char*)buffer;

    if (strncmp((char*)buffer, "blob", type_len) == 0)
        *type_out = OBJ_BLOB;
    else if (strncmp((char*)buffer, "tree", type_len) == 0)
        *type_out = OBJ_TREE;
    else
        *type_out = OBJ_COMMIT;
